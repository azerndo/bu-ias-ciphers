...
class ChaCha20 {
  ...
  generateKeystream(length) {
      ...
      const nonceWords = [
          nonceBytes[0] | (nonceBytes[1] << 8) | (nonceBytes[2] << 16) | (nonceBytes[3] << 24),
          nonceBytes[4] | (nonceBytes[5] << 8) | (nonceBytes[6] << 16) | (nonceBytes[7] << 24),
          nonceBytes[8] | (nonceBytes[9] << 8) | (nonceBytes[10] << 16) | (nonceBytes[11] << 24)
      ];
      ...
      for (let i = 0; i < length; i += 64) {
          ...
          for (let j = 0; j < 16; j++) {
              ...
              if (pos + 1 < length) keystream[pos + 1] = (workingState[j] >>> 8) & 0xFF;
              if (pos + 2 < length) keystream[pos + 2] = (workingState[j] >>> 16) & 0xFF;
              if (pos + 3 < length) keystream[pos + 3] = (workingState[j] >>> 24) & 0xFF;
          }
      }
